...
# Iniciação da classe das físicas
class Physics():
    def __init__(self, sprites1, sprites2):
        ...
        self.smooth = 1
    # Função de atualização da física (Colisão, gravidade...)
    def update(self):
    ...
    def collision(self):
        ...
        for i in range(len(self.world)):
            ...
            if not limit:
                self.left = 5
                self.right = 5
    # Função que determina a altura máxima do pulo
    ...
    def gravityController(self):
        if not self.collide:
            ...
            self.fall = False
            self.gravity = self.min_gravity
    # Função para queda do personagem
    def objectFall(self):
        self.aux.append(self.character.rect.y)
        if len(self.aux) > 2:
            self.aux.clear()
